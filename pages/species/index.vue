...
async function insertSpecies(){
  ...
  try{
    const data = await $fetch(`/api/species`,{method:'POST',body: {
  ...
  <v-container>
        <v-row>
          <v-col cols="12" v-if="user">
            <v-btn density="compact" @click="diagInsert = true">insert a new species</v-btn>
          </v-col>
              ...
              <div class="d-inline-flex mx-2">
                <NuxtLink :to="`/species/${s.name}`">
                  <v-img
                    :width="100"
                    cover
                    :src="`https://pepperdatabase.org/images/species/${s.ID}.jpg`"
                  ></v-img>
                </NuxtLink>
              </div>
              <div class="d-inline-flex mx-2">
