...
const { data: xchangeActive } = await useFetch("/api/xchange/active");

const currentRegistration = ref(null);
const { data: registrations } = await useFetch("/api/xregistration/mine");
if (registrations.value) {
  currentRegistration.value = registrations.value.find(
    (r) => r.exchange == xchangeActive.value,
  );
}
...
<template>
  <v-container>
    ...
    <v-card class="pa-2 mb-2">
      Browse Xchange: <v-btn to="/accessions">Acessions</v-btn>
    </v-card>
    ...
    <v-card class="pa-2 mb-2">
      ...
      <p>
        ...
        <a href="https://discord.gg/QD44vtCusv"
          >https://discord.gg/QD44vtCusv</a
        >
      </p>
    </v-card>
  </v-container>
