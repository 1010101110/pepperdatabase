...
async function insertVariety(){
  ...
  try{
    const data = await $fetch(`/api/varieties`,{method:'POST',body: {
  ...
  <v-container>
        <v-row>
          ...
          <v-col cols="12" v-if="user">
            <v-btn density="compact" @click="diagInsert = true">insert a new variety</v-btn>
          </v-col>
        ...
        <v-dialog v-model="diagInsert" max-width="400">
          <v-card class="pa-2">
            ...
            <v-select v-model="selSpecies" label="species" :items="species" item-title="name" item-value="ID"></v-select>
