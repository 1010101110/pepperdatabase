...
export default defineEventHandler(async (event) => {
    ...
    if(result.success){
        ...
        if(result.fields && result.fields.accession){
            const aID = Array.isArray(result.fields.accession) ? result.fields.accession[0] : result.fields.accession
            const [aresp] = await db.execute('SELECT * FROM xaccession where ID = ?',[aID])
            ...
            if(row){
                try {
                    ...
                    await db.execute('UPDATE xaccession SET images = ? where ID = ?',[JSON.stringify(row.images),aID]);
                } catch (err) {
